...
class trimesh:
    ...
    def __init__(self, r, lf, info=True):
        ...
        if len(self._non_manifold_edges) > 0:
            ...
            for k, cells in self._non_manifold_edges.items():
                ...
                print(
                    "  {0} cells share edge {1} located at {2} {3} {4}".format(
                        2 + len(cells), k + 1, *pt
                    )
                )
        ...
        if info:
            ...
            print(
                indent_level + "  # of vertices = {0} ({1})".format(self.np, self.nbp)
            )
            print(
                indent_level + "  # of edges    = {0} ({1})".format(self.ne, self.nbe)
            )
            print(
                indent_level + "  # of faces    = {0} ({1})".format(self.nf, self.nbf)
            )
            ...
            if edge_marker[i] < 0:
                        ...
                        if self.bf[face]:
                            for l in range(3):
                                ...
                                if (edge_marker[k] >= 0) or (not self.be[k]):
                                    continue
                                if np.any(self.le[k, :] == next_pt) and np.all(
                                    self.le[k, :] != last_pt
                                ):
                ...
                for i in range(cycle_tmp.shape[0]):
                    ...
                    for k in range(self.kpf[next_pt], self.kpf[next_pt + 1]):
                        ...
                        for l in range(3):
                            if not (
                                self.bp[self.lf[face, l]]
                                and (self.lf[face, l] in cycle_tmp)
                            ):
                                break
                        else:
                    ...
                    raise ValueError(
                        "Could not find suitable starting point for boundary cycle"
                    )
                if i > 0:
    ...
    def merge_cells(self, eflag):
        ...
        return cell_group, np.where(
            cell_group[self.lef[:, 0]] != cell_group[self.lef[:, 1]]
        )[0]
...
def compute_greedy_homotopy_basis(face, vertex, bi, face_sweight=None):
    ...
    V[1::3] = np.arange(nf)
    V[2::3] = np.arange(nf)
    amd = scipy.sparse.csc_matrix(
        (V + 1, (I, J)), shape=(3 * nf, 3 * nf), dtype=np.int32
    )
    am = amd.copy()
    am.data.fill(1)
    am += am.transpose()
    ...
    eif2 = eif[:, ind]
    Gs = scipy.sparse.csc_matrix(
        (np.ones((eif2.shape[1],)), (eif2[0, :], eif2[1, :])), shape=(nf, nf)
    )
    Gs += Gs.transpose()
    ...
    V = -(
        (distance[ei[0, :]] + distance[ei[1, :]]).flatten()
        + np.linalg.norm(dvi[0, :, :], axis=1)
    )
    ...
    while True:
        ...
        for i in range(1, cycle.shape[0] - 1):
            ...
            for k in range(mesh.kpf[cycle[i]], mesh.kpf[cycle[i] + 1]):
                face = mesh.lpf[k]
                if (cycle[i - 1] in mesh.lf[face, :]) and (
                    cycle[i + 1] in mesh.lf[face, :]
                ):
                    break
            else:
                continue
                ...
                for j in (-1, 1):
                    if cycle[i + j] in boundary_cycles[bCycle][2:-2]:
                        ...
                        if sCycle > len(boundary_cycles[bCycle]) / 2:
                            ...
                            insert_loop = np.flip(
                                boundary_cycles[bCycle][: sCycle + 1]
                            ).tolist()
                        if j < 0:
                            cycle = np.array(
                                cycle[: i + j].tolist()
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help='Ouput mesh file (default: Input file name with "-homology" appended)',
)
parser.add_argument(
    "--keep_nodeset_start",
    type=int,
    default=None,
    help="Starting index of nodesets to keep from input file",
)
parser.add_argument(
    "--plot_final",
    action="store_true",
    default=False,
    help="Show final homology basis?",
)
parser.add_argument(
    "--plot_steps",
    action="store_true",
    default=False,
    help="Show intermediate bases for each distinct surface?",
)
parser.add_argument(
    "--show_omitted",
    action="store_true",
    default=False,
    help="Show boundary cycles that are omitted?",
)
parser.add_argument(
    "--show_covering",
    action="store_true",
    default=False,
    help="Show covering triangles for boundary cycles (only used if `--plot_steps`)?",
)
parser.add_argument(
    "--debug",
    action="store_true",
...
parser.add_argument(
    ...
    type=float,
    nargs="+",
    help="Reference location for base point [x,y,z] (default: [0,0,0])",
)
parser.add_argument(
    "--optimize_holes",
    action="store_true",
    default=False,
    help="Sample additional points to attempt to optimize holes?",
)
...
with h5py.File(options.in_file) as fid:
    ...
    if options.keep_nodeset_start is not None:
        if "mesh/NUM_NODESETS" not in fid:
            parser.exit(
                -1, '"--keep_nodeset_start" specified but no nodesets available'
            )
        ...
        if options.keep_nodeset_start < 0:
            if options.keep_nodeset_start < -num_nodesets:
                parser.exit(
                    -1,
                    '"--keep_nodeset_start" exceeds the number of available nodesets',
                )
            options.keep_nodeset_start = num_nodesets + 1 + options.keep_nodeset_start
        if options.keep_nodeset_start > num_nodesets:
            parser.exit(
                -1, '"--keep_nodeset_start" exceeds the number of available nodesets'
            )
        for j in range(num_nodesets):
            if j + 1 >= options.keep_nodeset_start:
                try:
                    keep_nodesets.append(
                        np.asarray(fid["mesh/NODESET{0:04d}".format(j + 1)]) - 1
                    )
                except:
...
for surf_id in range(np.max(full_mesh.surf_tag) + 1):
    print()
    print(
        "Analyzing surface {0} of {1}".format(
            surf_id + 1, np.max(full_mesh.surf_tag) + 1
        )
    )
    ...
    print(
        "    Euler Characteristic (covered) = {0} ({1})".format(
            mesh.np - mesh.ne + mesh.nf,
            mesh.np - (mesh.ne + new_ne) + (mesh.nf + new_nf),
        )
    )
    ...
    # TEST MODE: Include ALL 64 port boundaries as holes (do not skip longest)
    # Original code skipped the longest boundary to avoid redundancy
    # This test includes all 66 holes (64 ports + 2 homology bases)
    for cycle in boundary_cycles[surf_id]:
        holes.append(cycle)  # Include ALL boundaries, including longest
    ...
    if options.optimize_holes and (len(hb) > 0):
        ...
        for j in range(len(hb)):
            ...
            hb2 = compute_greedy_homotopy_basis(
                face_covered, vertex, ind2, face_sweight=mesh.nf
            )
            ...
            print(
                indent_level
                + "[{2}/{3}] Reducing mesh to {0} macro cells with {1} edges".format(
                    ncoarse, keep_edges.shape[0], j + 1, len(hb)
                )
            )
    ...
    if options.plot_steps:
        ...
        if options.show_covering and (len(new_lc) > 0):
            ax.plot_trisurf(
                vertex[:, 0], vertex[:, 1], vertex[:, 2], triangles=new_lc, color="g"
            )
        for k, cycle in enumerate(new_bcycles):
            if k == cycle_max[1]:
                if options.show_omitted:
                    ax.plot(
                        vertex[cycle, 0], vertex[cycle, 1], vertex[cycle, 2], color="k"
                    )
            else:
                ax.plot(
                    vertex[cycle, 0],
        ...
        for basis_cycle in hb_out:
            ax.plot(
                vertex[basis_cycle, 0],
                vertex[basis_cycle, 1],
                vertex[basis_cycle, 2],
                color="tab:orange",
            )
...
with h5py.File(out_file, "r+") as h5_file:
    ...
    if len(nodesets) > 0:
        ...
        for k, nodeset in enumerate(nodesets):
            j += 1
            h5_file.create_dataset(
                "mesh/NODESET{0:04d}".format(j), data=nodeset + 1, dtype="i4"
            )
    # Replace sidesets
    ...
    if len(closures) > 0:
        ...
        h5_file.create_dataset(
            "mesh/SIDESET{0:04d}".format(1), data=closures, dtype="i4"
        )
...
if options.plot_final:
    ...
    for closure_cell in closures:
        ...
        ax.plot(
            vertex_full[closure, 0],
            vertex_full[closure, 1],
            vertex_full[closure, 2],
            "o",
            color="k",
        )
    ax.set_aspect("equal", "box")
