...
int main(int argc, char* argv[]) {
    ...
    thincurr_td_init(
        tw_obj_ptr, false, dt, 1.0E-6, true, status_freq, plot_freq,
    ...
    // Probe point(s) for magnetic field evaluation
    const int32_t n_probe_pts = 1;
    std::vector<double> probe_pts(3 * n_probe_pts, 0.0);
    std::vector<double> probe_field(3 * n_probe_pts, 0.0);
    std::ofstream probe_out("probe_field.csv");
    probe_out << "step,time,Bx,By,Bz\n";
    // Example probe point (R, Z, phi) or Cartesian (x, y, z) depending on mesh
    probe_pts[0] = 3.0; // x
    probe_pts[1] = 0.0; // y
    probe_pts[2] = 0.0; // z
    for (int istep = 0; istep < nsteps; ++istep) {
        ...
        // Evaluate magnetic field at probe point(s)
        thincurr_td_eval_field_points(
            td_state_ptr, n_probe_pts, probe_pts.data(), probe_field.data(), error_str
        );
        if (check_error(error_str, "thincurr_td_eval_field_points")) return -1;
        if (probe_out.good()) {
            probe_out << istep << "," << t << "," << probe_field[0] << ","
                      << probe_field[1] << "," << probe_field[2] << "\n";
        }
        if (istep % status_freq == 0) {
            log_info("B@probe = (" + std::to_string(probe_field[0]) + ", " +
                     std::to_string(probe_field[1]) + ", " +
                     std::to_string(probe_field[2]) + ")");
        }
    }
    probe_out.close();
