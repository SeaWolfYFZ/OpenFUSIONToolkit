...
├── compile.sh                  # Build script (supports both methods)
...
# Or manually (note: tokamak-test-10.cpp is not included in this example)
# This method is only for reference from ThinCurr-C-example-10
g++ -std=c++11 -o tokamak-test src/main.cpp \
  -I./include \
  -L/home/yfz/project/OpenFUSIONToolkit-251221/install_release/bin \
  -Wl,-rpath,/home/yfz/project/OpenFUSIONToolkit-251221/install_release/bin \
  -loftpy
```
...
First, create a Fortran namelist configuration file with the name `oftcppin` (hardcoded in the program). You can use the example:
```bash
cp config_example.in oftcppin
...
- `compile.sh`: Dual-mode build script supporting both CMake and direct compilation
